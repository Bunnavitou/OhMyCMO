...
import {
  LayoutGrid, List, Search, Calendar, User, AlertTriangle, Clock,
...
import {
  TASK_STATUSES, statusStyle, priorityStyle, dueBucket, dueTextStyle, collectTasks,
} from '../utils/tasks.js'
...
export default function Tasks() {
  ...
  const scopedTasks = useMemo(
    () => (scope === 'mine' && user?.id ? allTasks.filter((t) => t.assigneeId === user.id) : allTasks),
    [allTasks, scope, user],
  )
  ...
  }, [scopedTasks, q, assignee, due, source])
  ...
  const activity = useMemo(() => {
    ...
    for (const c of state.customers || []) {
      for (const l of c.logs || []) logs.push({ ...l, ownerName: c.name, link: `/customers/${c.id}` })
    }
    return logs
  ...
  const setStatus = (task, status) => {
    ...
    else updatePartnerTask(task.ownerId, task.taskId, { done: status === 'Done' })
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex rounded-xl border border-shadow overflow-hidden w-fit">
        <button
          ...
          className={`px-4 py-2 text-sm font-semibold ${scope === 'mine' ? 'bg-charcoal text-white' : 'text-graphite hover:bg-iron'}`}
        >
          {t('tasks.scope.mine')}
        </button>
        <button
          onClick={() => setScope('team')}
          className={`px-4 py-2 text-sm font-semibold ${scope === 'team' ? 'bg-charcoal text-white' : 'text-graphite hover:bg-iron'}`}
        >
          {t('tasks.scope.team')}
        </button>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="flex rounded-xl border border-shadow overflow-hidden">
          <button
            ...
            className={`px-3 py-2 flex items-center gap-1.5 text-sm font-semibold ${view === 'board' ? 'bg-charcoal text-white' : 'text-graphite hover:bg-iron'}`}
          >
            <LayoutGrid className="w-4 h-4" /> {t('tasks.view.board')}
          </button>
          <button
            onClick={() => setView('table')}
            className={`px-3 py-2 flex items-center gap-1.5 text-sm font-semibold ${view === 'table' ? 'bg-charcoal text-white' : 'text-graphite hover:bg-iron'}`}
          >
      ...
      {filtered.length === 0 ? (
        <div className="card text-center py-12">
          <p className="text-sm text-graphite">{t('tasks.empty')}</p>
        </div>
      ) : view === 'board' ? (
        ...
        <TaskTable tasks={filtered} onSetStatus={setStatus} />
      )}
              ...
              <li key={l.id}>
                <Link to={l.link} className="card !p-3 flex items-start gap-3 hover:scale-[1.01] transition-transform">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] text-graphite mt-0.5">{l.ownerName}</p>
                  </div>
...
function TaskCard({ task, onDragStart, onDragEnd, onSetStatus }) {
  ...
  return (
    <div
      ...
      onDragEnd={onDragEnd}
      className="card !p-3 cursor-grab active:cursor-grabbing space-y-1.5"
    >
      ...
      <Link to={task.link} className="text-[11px] text-graphite hover:text-wise-dark inline-flex items-center gap-1">
        {task.ownerName} <ArrowUpRight className="w-3 h-3" />
      </Link>
      <div className="flex flex-wrap gap-x-3 gap-y-1 text-[11px]">
        ...
        {task.createdByName && (
          <span className="text-ash">by {task.createdByName}</span>
        )}
      </div>
        ...
        {TASK_STATUSES.map((s) => (
          <button
            ...
            className={`px-2 py-0.5 rounded-md text-[10px] font-medium border transition ${
              task.status === s ? `${statusStyle(s)} border-transparent` : 'border-shadow text-graphite hover:bg-iron'
            }`}
          >
            {s}
          </button>
...
function TaskTable({ tasks, onSetStatus }) {
  const { t } = useT()
          ...
          {sorted.map((task) => {
            ...
            return (
              <tr key={task.key} className="border-b border-shadow last:border-0 hover:bg-iron/60">
                ...
                <td className="p-3">
                  <Link to={task.link} className="text-graphite hover:text-wise-dark inline-flex items-center gap-1">
                    {task.ownerName} <ArrowUpRight className="w-3 h-3" />
                  </Link>
                </td>
